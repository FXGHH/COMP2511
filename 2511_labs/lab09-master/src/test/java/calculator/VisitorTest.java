...
public class VisitorTest {
    ...
    @Test
    public void testSimpleModExpression2Evaluator() {
        Parser parser = new Parser("5 % 10 + 3 * 8 - 6"); 
	    TreeNode node = parser.parse();
        EvaluatorVisitor evaluatorVisitor = new EvaluatorVisitor();
        node.accept(evaluatorVisitor);
        assertEquals(23, evaluatorVisitor.getValue());
    }

    @Test
    public void testSimpleModExpression2Infix() {
        Parser parser = new Parser("5 % 10 + 3 * 8 - 6"); 
	    TreeNode node = parser.parse();
        Visitor infixVisitor = new InFixPrintVisitor();
        node.accept(infixVisitor);
        assertEquals("(((5 % 10) + (3 * 8)) - 6)", out.toString().strip());
    }
}