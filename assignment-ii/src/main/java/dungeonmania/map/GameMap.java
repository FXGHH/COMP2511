...
public class GameMap {
    ...
    private void initRegisterMovables() {
        ...
        enemies.forEach(e -> {
            game.register(() -> checkMove(e), Game.AI_MOVEMENT, e.getId());
        });
    }

    private void checkMove(Enemy e) {
        if (e instanceof Mercenary && ((Mercenary) e).isAllied() == true) {
            e.move(game);
        } else {
            if (e.checkNeedDwell(game.getMap()) == false) {
                e.move(game);
            }
        }
    }
    ...
    private void triggerOverlapEvent(Entity entity) {
        ...
        if (!(entity instanceof Player)) {
            ...
            getEntities(entity.getPosition()).forEach(e -> {
                if (e != entity)
                overlapCallbacks.add(() -> p.pickUp(e, this));
            });
        }
